...
//printf("\nin cardEffect\n");
  ...
  switch( card ) 
    {
		case adventurer:
			return adventurerAction(handPos, currentPlayer, drawntreasure, state);
		
		case baron:
			baronAction(choice1, currentPlayer, state);
		
		case outpost:
			outpostAction(handPos, currentPlayer, state);
		
		case smithy:
			smithyAction(handPos, currentPlayer, state);	
		
		case steward:
			stewardAction(choice1, choice2, choice3, handPos, currentPlayer, state);
...
//----------------------------------------------------------------------------------
//---------------------Added refactored code for assignment 2-----------------------
//----------------------------------------------------------------------------------
...
{  
	...
	while(drawntreasure<4){		//changed from a 2 to a 4 here
...
int baronAction(int choice1, int currentPlayer, struct gameState *state)
{
...
int outpostAction(int handPos, int currentPlayer, struct gameState *state)
{    
 	 //set outpost flag
      state->outpostPlayed++;
			
      //discard card
      discardCard(handPos, currentPlayer, state, 1);	//changed 0 to 1 here
      return 0;
}	
...
//printf("hand count in saction before draw: %d\n", state->handCount[currentPlayer]);
      ...
      //+3 Cards
      for (i = 0; i < 2; i++)	//changed 3 to 2 here
		{
		  drawCard(currentPlayer, state);
		}
...
int stewardAction(int choice1, int choice2, int choice3, int handPos,
	 int currentPlayer, struct gameState *state)
{
      if (choice1 == 1)
	{
      ...
      else if (choice1 == 3)	//changed from 2 to 3 here
	{
	  //+2 coins
	  state->coins = state->coins + 2;
	}
